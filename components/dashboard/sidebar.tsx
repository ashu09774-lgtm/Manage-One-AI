  ...
  return (
    <>
      ...
      <aside
        ...
        className={cn(
          "fixed left-0 top-0 z-50 flex h-screen w-[min(18rem,82vw)] flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground shadow-sm transition-transform duration-300 lg:z-40 lg:transition-all",
      ...
      <div className="flex h-14 items-center justify-between border-b border-sidebar-border px-3">
        <Link href="/dashboard" className="flex items-center gap-2" onClick={closeMobile}>
          ...
          <span className={cn("truncate text-base font-bold", collapsed && "lg:hidden")}>Manage One AI</span>
        </Link>
        <Button
          variant="ghost"
          size="icon"
      ...
      <div className="p-3">
        <Button 
          className={cn("h-10 w-full gap-2", collapsed && "lg:px-0")}
          size={collapsed ? "icon" : "default"}
          asChild
        >
          <Link href="/dashboard/workspaces" onClick={closeMobile} aria-label="Create or manage workspaces">
            <Plus className="h-4 w-4" />
            <span className={cn(collapsed && "lg:hidden")}>New Workspace</span>
          </Link>
        </Button>
      </div>
